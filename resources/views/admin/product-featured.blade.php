...
@endif
      <!--  <input type="" name="keyword" placeholder="Enter State Name. .">
           <input type="submit" class="btt" value="Filter"> -->
                        ...
                        <form class="form-pro"  action="{{route('admin.product-featured')}}" method="post">
                            ...
                            <tbody>
							 @foreach($products as $product)
                                <tr>
									...
									<td >
						 <select class="form-control change_status"  name="change_feature" data-id="{{$product->id}}">
							
							<option @if($product->featured==1) style="color:red;" {{'selected'}}  @endif value="1">Yes</option>
							<option @if($product->featured==0) {{'selected'}} @endif value="0">No</option>
                                    ...
                                    <td>
									<a class="action-dash" href="{{route('admin.product.edit',['id'=>$product->id])}}" ><i class="fa fa-pencil" aria-hidden="true"></i>
...
$(document).ready(function() {
		  $("select[name='change_feature']").change(function(){
		  ...
		  if(row_id && option_value){
			$.ajax({
           type: "GET",
           url:"{{route('ajaxUpdateFeature')}}?id="+row_id+"&status="+option_value,

...
class ListingController extends Controller
{
	...
	public function getFilterListings(Request $request){
		...
		$listings=Category::getAllListings();			
			...
			
			{
				...
				$filter_location = array_filter($filter_location);
				$filter_category = array_filter($filter_category);
				
				
				
				
				$listings=Listing::Category($filter_category)->State($filter_location)->Title($filter_title)->Description($filter_title)->get();
 ...
 public function getAjaxFilterListings(Request $request){
	
		$data[]=array();
		
		$data['subCategory']=Category::where('parent_id','=',0)->where('status','=',1)->get();
		
		
				
				$filter_location = $request->input('location_data');
				$filter_category = $request->input('category_data');
				
				$filter_location =explode(',',$filter_location);
				$filter_category =explode(',',$filter_category);
				
				
		$data['listings']=Listing::Category($filter_category)->State($filter_location)->get();
			
			$returnHTML = view('front.list-data',$data)->render();
			
			
			
			return response()->json(array('status' => 200, 'success' => true, 'html' => $returnHTML));
	...
	public function index(Request $request,$id){
		...
		return view('front.list_detail',compact('listing','listings'));
